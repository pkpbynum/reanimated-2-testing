...
class DraggableViewManager {
  ...
  get _onSwitch() {
    return (prevIdx, currIdx) => {
      'worklet'
      this.offsets.value[this.ids.value[currIdx]].value =
        prevIdx * this.itemHeight
  ...
  _generateGestureHandler = (_id, _offset, _animating) => {
    return useAnimatedGestureHandler({
      onActive: (e, ctx) => {
        ...
        if (ctx.prevIdx !== currIdx) {
          this._onSwitch(ctx.prevIdx, currIdx)
        }
        ctx.prevIdx = currIdx
      },
      onEnd: () => {
  ...
  _onDraggableLayout = (_id, _index, _offset) => {
    _offset.value = _index * this.itemHeight
    this.initialOffsets[_id] = _offset
    this.numItems++

    if (this.numItems === this.totalItems || this.initialized.value) {
      // Copy initialOffsets to avoid locking it to readonly
      this.offsets.value = {...this.initialOffsets}
